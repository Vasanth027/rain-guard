...
function showCoordinates(lat, lon, accuracy = null) {
  ...
  const latitude = state.lat.toFixed(6);
  const longitude = state.lon.toFixed(6);
  ...
  const point = [state.lat, state.lon];
  if (!state.map) {
    state.map = L.map('locationMap', { zoomControl: true, attributionControl: true }).setView(point, 17);
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
      maxZoom: 19,
      attribution: '&copy; OpenStreetMap contributors'
    }).addTo(state.map);
  } else {
    state.map.setView(point, 17, { animate: true });
  }

  if (state.marker) state.map.removeLayer(state.marker);
  if (state.accuracyCircle) state.map.removeLayer(state.accuracyCircle);

  // Google-style blue current-location dot with a GPS accuracy ring.
  state.accuracyCircle = L.circle(point, {
    radius: Math.max(Number(accuracy) || 20, 8),
    color: '#4285f4',
    weight: 1,
    fillColor: '#4285f4',
    fillOpacity: 0.14
  ...
  state.marker.bindPopup(`<strong>📍 Your current location</strong><br>${latitude}, ${longitude}${accuracy ? `<br>Accuracy: ±${Math.round(accuracy)} m` : ''}`).openPopup();
...
async function loadWeather(lat, lon, label, accuracy = null) {
  ...
  // Reveal the dashboard BEFORE Leaflet initializes so the map has real dimensions.
...
$('locationBtn').addEventListener('click', () => {
  ...
  navigator.geolocation.getCurrentPosition(async ({ coords }) => {
    try { await loadWeather(coords.latitude, coords.longitude, 'My Exact Location', coords.accuracy); }
    catch (error) { setStatus(error.message); }
  }, (error) => {
    setStatus(error.code === 1 ? 'Location access was denied. Allow location permission and try again.' : 'Unable to get your GPS location. Try again.');
...
$('copyCoordinates').addEventListener('click', async () => {
  ...
  try { await navigator.clipboard.writeText(value); setStatus(`Coordinates copied: ${value}`); }
  catch (_) { setStatus(`Coordinates: ${value}`); }
});
...
setInterval(async () => {
  if (state.lat !== null && state.lon !== null) {
    try { await loadWeather(state.lat, state.lon, state.city); } catch (_) {}
  }
}, 15 * 60 * 1000);
